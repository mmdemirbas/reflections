...
class MethodParameterNamesScannerTest {
    ...
    inner class ParamNames {
        private val scanner = MethodParameterNamesScanner()
            .scan(TestModel::class.java).dump()

        @Test
        fun ctor() {
            assertToStringEqualsSorted(listOf("f1"),
                                                       scanner.paramNames(TestModel.C4::class.java.getDeclaredConstructor(
                                                               String::class.java)))
        }
        ...
        fun `multiple params`() {
            assertToStringEqualsSorted(listOf("i1", "i2"),
                                                       scanner.paramNames(TestModel.C4::class.java.getDeclaredMethod(
                                                               "add",
