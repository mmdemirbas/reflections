...
public class VfsTest {
    ...
    public void allKindsOfShittyUrls() throws Exception {
...

        {
            ...
            Dir      dir  = DefaultUrlTypes.jarUrl.createDir(rtJarUrl);
            Vfs.File file = null;
            for (Vfs.File f : dir.getFiles()) {
...

        {
            ...
            Dir      dir  = DefaultUrlTypes.directory.createDir(thisUrl);
            Vfs.File file = null;
            for (Vfs.File f : dir.getFiles()) {
        ...
        Dir dir = DefaultUrlTypes.directory.createDir(tempFile.toURL());
        assertNotNull(dir);
    ...
    public void vfsFromDirWithJarInName() throws MalformedURLException {
        ...
        try {
            Dir dir = Vfs.fromURL(new URL(format("file:{0}", dirWithJarInName)));
    ...
    public void vfsFromDirWithinAJarUrl() throws MalformedURLException {
        ...
        Dir dir = Vfs.fromURL(new URL(directoryInJarPath));
    ...
    @Test
    public void findFilesFromEmptyMatch() {
        URL                jar   = getSomeJar();
        Iterable<Vfs.File> files = Vfs.findFiles(Collections.singletonList(jar), Predicates.alwaysTrue());
        assertNotNull(files);
    ...
    public void vfsFromHttpUrl() throws MalformedURLException {
        Vfs.addDefaultURLTypes(new UrlType() {
            ...
            @Override
            public Dir createDir(URL url) {return new HttpDir(url);}
        });
    ...
    static class HttpDir implements Dir {
