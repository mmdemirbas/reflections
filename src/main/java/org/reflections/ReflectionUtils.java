...
public abstract class ReflectionUtils {
    ...
    public static Set<Method> getMethods(Class<?> t, Predicate<? super Method>... predicates) {
        return t.isInterface() ? filter(t.getMethods(), predicates) : filter(t.getDeclaredMethods(), predicates);
    }
    ...
    public static Set<Constructor> getConstructors(Class<?> t, Predicate<? super Constructor>... predicates) {
        return filter(t.getDeclaredConstructors(), predicates); //explicit needed only for jdk1.5
    }
    ...
    public static Predicate<Member> withAnyParameterAnnotation(Class<? extends Annotation> annotationClass) {
        return input -> (input != null) && Iterables.any(annotationTypes(parameterAnnotations(input)),
                                                         input1 -> input1.equals(annotationClass));
    }
    ...
    public static Predicate<Member> withAnyParameterAnnotation(Annotation annotation) {
        return input -> (input != null) && Iterables.any(parameterAnnotations(input),
    ...
    public static Class<?> forName(String typeName, ClassLoader... classLoaders) {
        if (getPrimitiveNames().contains(typeName)) {
            ...
            if (typeName.contains("[")) {
                ...
                type = getPrimitiveNames().contains(type) ? getPrimitiveDescriptors().get(getPrimitiveNames().indexOf(
                        type)) : ("L" + type + ";");
            ...
            for (ClassLoader classLoader : ClasspathHelper.classLoaders(classLoaders)) {
                if (type.contains("[")) {
                    try {
                        ...
                        reflectionsExceptions.add(new ReflectionsException("could not get type for name " + typeName,
                                                                           e));
                    }
                }
                try {
    ...
    private static Class[] parameterTypes(Member member) {
        if (member == null) {
            ...
            return (member instanceof Executable) ? ((Executable) member).getParameterTypes() : null;
        }
    }
    ...
    private static Set<Annotation> parameterAnnotations(Member member) {
        ...
        annotations = (member instanceof Executable) ? ((Executable) member).getParameterAnnotations() : null;
    ...
    private static void initPrimitives() {
        if (primitiveNames == null) {
            primitiveNames = Lists.newArrayList("boolean",
                                                "char",
                                                "byte",
                                                "short",
                                                "int",
                                                ...
                                                "double",
                                                "void");
            primitiveTypes = Lists.newArrayList(boolean.class,
                                                char.class,
                                                byte.class,
                                                short.class,
                                                int.class,
        ...
        if ((annotation2 != null) && (annotation1.annotationType() == annotation2.annotationType())) {
            for (Method method : annotation1.annotationType().getDeclaredMethods()) {
                try {
                    ...
                    throw new ReflectionsException(String.format("could not invoke method %s on annotation %s",
                                                                 method.getName(),
    ...
    private static boolean isAssignable(Class[] childClasses, Class[] parentClasses) {
        ...
        for (int i = 0; i < childClasses.length; i++) {
            if (!parentClasses[i].isAssignableFrom(childClasses[i]) || ((parentClasses[i] == Object.class) && (
